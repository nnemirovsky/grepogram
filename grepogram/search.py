...
The condition is re-derived here rather than flagged anywhere: the automatic sync inside a
``search`` call passes ``recut=False`` (:func:`grepogram.sync.sync_all`) and so never advances
the re-cut, and an MCP-only user is exactly who never sees the log line. This warning is what
sends them to ``grepogram sync`` or to an explicit ``sync`` tool call, which does."""
