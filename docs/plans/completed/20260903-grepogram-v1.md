...
- **Fusion**: RRF `score = Σ 1/(rrf_k + rank)` over unit ids from `[lexical_units, lexical_msgs→window, dense]`; top `rerank_top` → cross-encoder `(query, unit.text)` → sort; dedup: drop a hit whose `msg_ids` overlap ≥ `dedup_overlap` with a higher-scored hit (overlap = the share of the hit's own `msg_ids` already carried by a kept hit of the same chat, so a thread inside a better-ranked window goes and a window extending a better-ranked thread stays; a value above 1.0 disables dedup); return top `k`. `mode` only selects the retrieval lists — reranking (`rerank=True`) and dedup apply in every mode.
...
- **Degradation**: `ModelUnavailable`, missing or empty `unit_vec`, or `EmbeddingSpaceMismatch` (checked read-only against `meta.embed_model`/`embed_dim` before every KNN, so a same-width model change is caught too) → `mode=lexical`, `warnings += ["dense search unavailable: <reason>"]`; the embedder is not loaded while there are no vectors to search; a reranker that fails to load adds `"reranking unavailable: <reason>"` and the fused order stands; no MPS → CPU with one-time WARN log.
...
- [x] `rrf(rankings: list[list[int]], k) -> dict[int, float]` (exists in `search.py` since task 17, where lexical mode already fuses the unit and message lists; extend its test with the three-list case); `dedup(hits, overlap) -> list[Hit]` by `msg_ids` overlap against higher-scored hits
- [x] `search(...)` for `mode="hybrid"|"dense"`: lexical lists + dense list → `rrf` → top `rerank_top` → rerank (when `rerank=True` and a reranker loads) → dedup → top `k`; `ModelUnavailable` or empty/missing `unit_vec` → fall back to lexical with `warnings`; empty `fts_query` in hybrid → dense-only
- [x] CLI: `--mode hybrid|dense`, `--no-rerank`; hybrid becomes the default
- [x] write tests: `rrf` math on a hand-computed example; `dedup` keeps higher score; hybrid finds the fixture's synonym paraphrase that lexical misses; dense unavailable → lexical result + warning; hybrid on a never-embedded DB → lexical hits + warning, no exception; `rerank=False` skips reranker
